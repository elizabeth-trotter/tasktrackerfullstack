...
const NavbarComponent = ({ userColor }: NavbarProps) => {
  return (
    <nav className='h-24 bg-gradient-to-r from-[#AEE6D9] to-[#3EBE9F] flex justify-between items-center px-5 font-[HammersmithOne]'>
      ...
      <Dropdown label="" dismissOnClick={false} renderTrigger={() => <span className='border border-black rounded-full h-12 w-12' style={{ background: userColor?.startsWith('#') ? userColor : `url(${userColor})`, backgroundPosition: 'center', backgroundSize: 'contain'}}></span>}>
